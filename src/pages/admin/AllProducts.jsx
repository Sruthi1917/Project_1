...
const AllProducts = () => {
  ...
  const fetchData = async () => {
    await axios
      .get("http://localhost:6001/api/products/fetch-products")
      .then((response) => {
  ...
  const handleSortFilterChange = (e) => {
    ...
    if (value === "low-price") {
      ...
      setVisibleProducts(
        visibleProducts.sort((a, b) => b.discount - a.discount)
      );
    }
  };
  ...
  useEffect(() => {
    if (categoryFilter.length > 0 && genderFilter.length > 0) {
      ...
      setVisibleProducts(
        products.filter((product) => genderFilter.includes(product.gender))
      );
    } else if (categoryFilter.length > 0 && genderFilter.length === 0) {
      setVisibleProducts(
